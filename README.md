...
- **Reprise de l'existant** : choisis ton `secrets.yaml` de Home Assistant ou
  ton `.env` dans l'interface, ou lance `synsec import Maison secrets.yaml`.
...
```
go build -ldflags "-X synsec/internal/web.SourceURL=https://git.exemple.fr/moi/synsec" ./cmd/synsec
```
